 ...
 */
@Controller
@RequestMapping(value = "api/rest/imgscalrjava/*")
public class ImgscalrJavaResource extends BaseRestResource implements InterfaceRessource {

    /**
     * Logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger(ImgscalrJavaResource.class);

    @Inject
    private BildService imgscalrJavaBildService;

    /**
     * Rest-Service welcher mit dem Path zum Bild aufgerufen wird. Er das Bild
     * zurück. Er ist mit Breite und Hoehe.
     * 
    ...
    public void scaleImage(final HttpServletResponse response, @PathVariable("file_path") final String filePath,
        ...
        try {
            imgscalrJavaBildService.scaleFile(SLASH + filePath + SLASH + filePath2 + SLASH + file + POINT + fileEnd,
