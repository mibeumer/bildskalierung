 ...
 */
@Controller
@RequestMapping(value = "api/rest/talkjava/*")
public class TalkJavaResource extends BaseRestResource implements InterfaceRessource {

    /**
     * Logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger(TalkJavaResource.class);

    @Inject
    private BildService talkJavaBildService;

    /**
     * Rest-Service welcher mit dem Path zum Bild und der Hoehe und Breite
     * aufgerufen wird. Er das Bild zurück.
     * 
    ...
    public void scaleImage(final HttpServletResponse response, @PathVariable("file_path") final String filePath,
        ...
        try {
            talkJavaBildService.scaleFile(SLASH + filePath + SLASH + filePath2 + SLASH + file + POINT + fileEnd, width,
                    height, response.getOutputStream());
        } catch (IOException e) {
