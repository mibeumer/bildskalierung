 ...
 */
@Controller
@RequestMapping(value = "api/rest/graphicsmagickperformant/*")
public class GraphicsMagickPerformantResource extends BaseRestResource implements InterfaceRessource {

    /**
     * Logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger(GraphicsMagickPerformantResource.class);

    @Inject
    private BildService graphicsMagickPerformantBildService;
     ...
     * /graphicsmagick/img-adb/34201026-00-00/inferno.jpg
     * 
     * @param fileName
     * @return
     * @throws IOException
    ...
    public void scaleImage(final HttpServletResponse response, @PathVariable("file_path") final String filePath,
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Dies ist der Pfad zur Datei: \"" + SLASH + filePath + SLASH + filePath2 + SLASH + file + POINT
                    + fileEnd + "\". Breite: " + width + " Hoehe: " + height);
        }
        ...
        try {
            graphicsMagickPerformantBildService.scaleFile(SLASH + filePath + SLASH + filePath2 + SLASH + file + POINT
                    + fileEnd, width, height, response.getOutputStream());
        } catch (IOException e) {
